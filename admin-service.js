...
/**
 * Verifies a Google/Firebase ID token server-side via Google TokenInfo API
 */
...
async function handleAdminRequest(pathname, req, res, getRequestBody) {
  ...
  if (pathname === '/api/admin/verify-identity') {
    try {
      ...
      if (!verifiedUser || !verifiedUser.emailVerified) {
        recordAudit('ADMIN_IDENTITY_DENIED', verifiedUser?.email || 'unknown', 'AUTH', clientIp, { reason: 'UNVERIFIED_EMAIL' });
        res.statusCode = 403;
        res.setHeader('Content-Type', 'application/json');
      ...
      if (verifiedUser.email !== ADMIN_ALLOWED_EMAIL) {
  ...
  if (pathname === '/api/admin/unlock') {
    try {
      ...
      const verifiedUser = await verifyFirebaseToken(idToken);
      if (!verifiedUser || verifiedUser.email !== ADMIN_ALLOWED_EMAIL) {
        res.statusCode = 403;
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({ success: false, error: 'Admin access denied.' }));
        return true;
